...
// 	// 保存服务状态到etcd
// 	ip := utils.LocalIP()
// 	g.L.Info("register local ip to etcd", zap.String("ip", ip))
...
// 			if err != nil {
// 				g.L.Error("register to etcd error", zap.Error(err))
// 			}
...
// 	if err != nil {
// 		g.L.Fatal("Get application node error", zap.Error(err))
// 	}
...
// 					if err != nil {
// 						g.L.Warn("parse etcd kv error", zap.Error(err))
// 						continue
// 					}
...
// 	if err != nil {
// 		g.L.Fatal("etcd init failed", zap.Error(err))
// 	}
// 	return &Etcd{cli}
// }
...
// 	load, err := strconv.ParseFloat(utils.Bytes2String(kv.Value), 64)
// 	if err != nil {
// 		return nil, fmt.Errorf("load error:%v,key:%s, val: %s", err, string(kv.Key), string(kv.Value))
// 	}

// 	if !bytes.HasPrefix(kv.Key, utils.String2Bytes(AppRootPath)) {
...
// 	srv := utils.Bytes2String(ks[0])
// 	ip := utils.Bytes2String(ks[1])
